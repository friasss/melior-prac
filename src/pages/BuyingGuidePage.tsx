...
const DOCS = [
  ...
  'Declaración de impuestos (IR-1 o IR-2)',
  'Referencias bancarias o extractos de cuenta',
  'Certificado de no deuda (DGII)',
  'Carta de pre-aprobación bancaria',
        ...
        <div className="bg-brand-600 px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-brand-100">Estimado mensual según banco dominicano</p>
            </div>
          </div>
        </div>
        ...
        <div className="p-6">
          ...
          <div className="mt-6 grid grid-cols-3 gap-3 rounded-2xl bg-brand-50 p-5 dark:bg-brand-950/40">
            <div className="text-center">
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Cuota mensual</p>
              <p className="mt-1 font-heading text-xl font-extrabold text-brand-700 dark:text-brand-300">{formatRD(monthly)}</p>
            </div>
            <div className="text-center">
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Monto del préstamo</p>
              <p className="mt-1 font-heading text-xl font-extrabold text-slate-700 dark:text-slate-300">{formatRD(loanAmt)}</p>
            </div>
            <div className="text-center">
              <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">Total intereses</p>
          ...
          <p className="mt-3 text-center text-xs text-slate-400">
            * Estimado referencial. Consulta con tu banco para condiciones reales.
          </p>
        </div>
      </div>
    </section>
      ...
      <section className="mx-auto max-w-3xl px-4 pb-12 sm:px-6">
        <div className="card p-6">
          <div className="flex items-center gap-3 mb-5">
            ...
            <div>
              <h2 className="font-heading text-lg font-bold text-slate-900 dark:text-white">Documentos Necesarios</h2>
              <p className="text-sm text-slate-500">Lista completa para el proceso de compra</p>
            </div>
          </div>
          <ul className="grid gap-2 sm:grid-cols-2">
            {DOCS.map((doc, i) => (
              <li key={i} className="flex items-start gap-2 text-sm text-slate-600 dark:text-slate-400">
                <span className="material-symbols-outlined text-brand-500 text-[16px] mt-0.5 shrink-0" style={{ fontVariationSettings: "'FILL' 1" }}>check_circle</span>
                {doc}
              </li>
            ))}
          </ul>
      ...
      <section className="mx-auto max-w-3xl px-4 pb-16 sm:px-6">
        <div className="card flex flex-col items-center gap-4 py-10 text-center">
          <div className="flex h-14 w-14 items-center justify-center rounded-2xl bg-brand-50 text-brand-600 dark:bg-brand-950 dark:text-brand-400">
            <span className="material-symbols-outlined text-2xl" style={{ fontVariationSettings: "'FILL' 1" }}>real_estate_agent</span>
          </div>
          <h3 className="font-heading text-xl font-bold text-slate-900 dark:text-white">¿Listo para dar el siguiente paso?</h3>
          <p className="max-w-md text-sm text-slate-500">
            Nuestros agentes inmobiliarios están disponibles para guiarte en todo el proceso de compra, desde la búsqueda hasta la firma.
          </p>
